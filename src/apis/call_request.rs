...
pub async fn call_gpt(messages: Vec<Message>) -> Result<String, Box<dyn std::error::Error + Send>> {
    ...
    let api_version: String =
        env::var("API_VERSION").expect("API_VERSION not found in environment variables");
    // Confirm endpoint
    ...
    let chat_completion: ChatCompletion = ChatCompletion {
        model: api_version.to_string(),
        messages,
        temperature: 0.1, // maybe wanna move to environment variable
    };

    // Troubleshooting for mod tests
    // let res_raw = client
    //   .post(url)
